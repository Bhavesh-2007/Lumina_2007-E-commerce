...
import { ShoppingBag, Search, Menu, X, Sun, Moon, UserRound, Heart, Store } from 'lucide-react';
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <div className="flex-shrink-0 flex items-center cursor-pointer group" onClick={() => setView('home')}>
            <Logo className="w-11 h-11 mr-2 transition-transform group-hover:scale-105" />
            <div className="flex flex-col -space-y-1">
              <span className="text-xl font-black bg-clip-text text-transparent bg-gradient-to-r from-orange-600 to-orange-400 dark:from-orange-400 dark:to-orange-200 tracking-tight">
                SnapCart
              </span>
              <span className="text-[10px] font-bold text-gray-400 dark:text-gray-500 uppercase tracking-[0.3em] ml-0.5">
                Shop
              </span>
            </div>
          </div>
          ...
          <div className="hidden md:flex flex-1 max-w-md mx-8 relative group">
            ...
            <input
              ...
              className="block w-full pl-10 pr-3 py-2 border border-transparent rounded-full leading-5 bg-gray-100 dark:bg-slate-800 text-gray-900 dark:text-gray-100 placeholder-gray-500 focus:outline-none focus:bg-white dark:focus:bg-slate-900 focus:border-orange-500 focus:ring-1 focus:ring-orange-500 sm:text-sm transition-all duration-300"
              placeholder="Search SnapCart..."
            />
          </div>

          {/* Desktop Nav */}
          <div className="hidden md:flex items-center space-x-6">
            <button onClick={() => setView('home')} className="text-gray-600 dark:text-gray-300 hover:text-orange-500 dark:hover:text-orange-400 font-medium transition-colors">Home</button>
            <button onClick={() => setView('shop')} className="text-gray-600 dark:text-gray-300 hover:text-orange-500 dark:hover:text-orange-400 font-medium transition-colors">Shop</button>
            <button 
          ...
          <div className="hidden md:flex items-center space-x-4">
            ...
            {user ? (
               <div className="flex items-center gap-2 cursor-pointer group" onClick={() => setView('profile')}>
                 <img src={user.avatar} alt="Profile" className="w-8 h-8 rounded-full border border-gray-300 object-cover transition-transform group-hover:scale-110" />
               </div>
            ) : (
              <button onClick={() => setView('login')} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 text-gray-600 dark:text-gray-300 group transition-all">
                <UserRound size={20} className="transition-transform group-hover:scale-125 group-hover:text-orange-500" />
              </button>
            )}

            <button onClick={onOpenCart} className="p-2 rounded-full hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors relative text-gray-600 dark:text-gray-300 group">
        ...
        <div className="md:hidden glass-panel border-t border-gray-200 dark:border-gray-800 absolute w-full z-50">
          <div className="px-4 py-3">
             <input
              ...
              value={searchQuery}
              onChange={handleSearch}
              className="block w-full px-3 py-2 border border-gray-300 dark:border-gray-600 rounded-md leading-5 bg-gray-50 dark:bg-slate-700 text-gray-900 dark:text-gray-100 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-orange-500"
              placeholder="Search..."
            />
          </div>
          <div className="px-2 pt-2 pb-3 space-y-1 sm:px-3">
            ...
            <button onClick={() => { setView('create-shop'); setIsMenuOpen(false); }} className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-purple-600 dark:text-purple-400 hover:bg-gray-100 dark:hover:bg-gray-800">Sell on SnapCart</button>
            ...
            {user ? (
               ...
               <button onClick={() => { setView('login'); setIsMenuOpen(false); }} className="block w-full text-left px-3 py-2 rounded-md text-base font-medium text-orange-500 hover:bg-gray-100 dark:hover:bg-gray-800">Log In</button>
            )}
          </div>
        </div>
      )}
